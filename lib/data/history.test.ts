...
describe("overlaySnapshots", () => {
  ...
  it("between-anchor buckets keep reconstructed compose values (no linear-blend overwrite)", () => {
    // User-reported #2: the previous linear-interp between snap
    // anchors overwrote the compose-derived reconstructed value
    // (which encodes market movement via quote-driven prices)
    // with a meaningless straight line. The new semantic keeps
    // the reconstructed shape between anchors and only pins at
    // exact snap times. Boundary continuity is achieved by
    // effectiveSnapNW computing the anchor as the same compose
    // value at snap.t.
    ...
    expect(out[0].netWorthUSD).toBe(1000);
    // Between anchors: KEEPS the reconstructed value (no blend).
    // (base series at t=200: 1100; at t=300: 1200.)
    expect(out[1].netWorthUSD).toBe(1100);
    expect(out[2].netWorthUSD).toBe(1200);
    // Post-last-anchor bucket: held at last anchor when no live-NW
    // pin is supplied.
  ...
  it("renders zero / negative-NW snapshots (user-intentional underwater state)", () => {
    ...
    // Bucket t=300 between [200, 1500] and [380, 0]: KEEPS the
    // reconstructed value from the base series (1200) — no more
    // linear interpolation between snap anchors.
    expect(out[2].netWorthUSD).toBe(1200);
  ...
  it("augments a snapshot's anchor NW with backdated live holdings missing from that snapshot (user-reported bug)", () => {
    ...
    // With the live-household augmentation:
    // - t=200 anchor adjusted to $50k snap + $100k backdated PRIV
    //   = $150k.
    // - t=300 (between anchors): keeps the RECONSTRUCTED value
    //   (baseSeries had 0). User can verify the chart breathes
    //   with market data between snaps now instead of holding
    //   flat at $150k.
    // - t=400 anchor: snap already had PRIV, no adjustment.
    ...
    expect(out[2].netWorthUSD).toBe(0); // t=300 keeps reconstructed
    expect(out[3].netWorthUSD).toBe(150_000); // t=400 anchor unchanged
  });

  it("liveNetWorth is a right-edge anchor; intermediate buckets keep reconstructed values", () => {
    // Snapshot at t=200 with NW=$1000, live NW=$2000 pinned at
    // last bucket t=400. Bucket at t=300 keeps its reconstructed
    // value (base series has 1200) — chart breathes with the
    // reconstructed shape between anchors. Both anchors still pin
    // exactly at their own t.
    ...
    expect(out[2].netWorthUSD).toBe(1200); // reconstructed (base)
