...
/**
 * Date picker for the History tab's CUSTOM range. The native
 * `<input type="date">` allows free-form typing — which is buggy
 * for partial inputs (a half-typed ISO string parses to `null`,
 * the chart's bucket bounds degenerate, and the SVG path math
 * NaNs out → crash). We sidestep this by blocking ALL keyboard
 * input on the input element (only the native calendar picker
 * can change the value) and surface a clearly visible calendar
 * button on the right.
 *
 * Behavior:
 *  - Reading: shows the formatted date or "Pick date" placeholder.
 *  - Typing/paste/cut: blocked at the keydown/paste/cut events.
 *  - Click anywhere on the input OR on the calendar button: opens
 *    the native picker via `showPicker()`. Falls back to focus()
 *    on the few browsers that don't support showPicker.
 *  - All onChange events still fire (driven by the picker), so the
 *    state stays in sync.
 */
function CustomDatePicker({
  ...
  const openPicker = () => {
    ...
    if (!el) return;
    // Prefer the modern showPicker API. It works on Chrome / Edge
    // / Firefox 101+ / Safari 17+ and explicitly opens the
    // calendar dropdown without grabbing keyboard focus.
    if (typeof el.showPicker === "function") {
      try {
        el.showPicker();
        return;
      } catch {
        // Some browsers throw if the input is hidden / unfocusable.
        // Fall back to focus() in that case.
      }
    }
    el.focus();
  };
  // Block ALL key input — the user can only change the value via
  // the calendar picker. Allow Tab/Shift+Tab/Esc so keyboard
  // navigation isn't trapped.
